...
export const levelMetadata = pgTable(
	...
	(table) => [
		index('IX_level_metadata_level').using(
			'btree',
...
export const levelPoints = pgTable(
	...
	(table) => [
		index('IX_level_points_level').using(
			'btree',
			table.idLevel.asc().nullsLast().op('int4_ops'),
		),
		foreignKey({
...
export const levelRequest = pgTable(
	...
	(table) => [
		...
		index('IX_level_request_workshop_id').using(
			'btree',
			table.workshopId.asc().nullsLast().op('numeric_ops'),
		),
	],
);
...
export const record = pgTable(
	...
	(table) => [
		...
		index('IX_records_level_time').using(
			'btree',
			table.idLevel.asc().nullsLast().op('int4_ops'),
			table.time.asc().nullsLast().op('int4_ops'),
		),
